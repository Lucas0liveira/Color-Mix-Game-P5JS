...
import { Hearts } from "../Hearts/index";
import {
    blendColors,
    subtractColorArrays,
    type Palette,
...
export function Stage(props: Props) {
    function handleAddColor(color: string) {
        const remaining = subtractColorArrays(props.solution, props.hits);

        if (remaining.includes(color)) {
            const newCurrentColor = blendColors([...props.hits, color]);
    ...
    return (
        <main className="stage">
            ...
            <div className="pallete-buttons">
                {Object.values(props.palette).map((color, index) => (
                    <ColorButton
                        key={index}
                        color={color}
            ...
            <Hearts lives={props.lives} maxLives={MAX_LIVES} />
        </main>
    );
}
